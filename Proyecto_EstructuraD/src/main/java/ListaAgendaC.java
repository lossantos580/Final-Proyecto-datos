...
//---------------------------------------------------------------------------------------------
   ...
   public void mostrarag1() {
        ...
        while (aux != fin2) {
            s += " \nDatos del animal: " +aux.getElementoDA().getDatosAnimal()+ 
               "\nID-Clinico animal: " + aux.getElementoDA().getIdA();
            
            aux = aux.getAnteriorDA();
        }
        JOptionPane.showMessageDialog(null, "Datos de Agenda de Cita: \n" + s);
    }
   
   public void mostrarag2() {
        String s = "";
        NodoDP aux = fin1;
            s +=  "\nNombre del cliente"+aux.getElementoPer().getNombre()+ "/"
